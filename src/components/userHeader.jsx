...
function UserHeader({ additionalBoldLink }) {
    ...
    const [hasCompletedLectures, setHasCompletedLectures] = useState(false);

    useEffect(() => {
        // localStorage에서 사용자 이름과 기이수 과목 등록 여부 확인
    ...
    const handleCompleteLectureClick = (e) => {
        e.preventDefault();
        // 기이수 과목 등록 여부에 따라 다른 페이지로 이동
        ...
        <header className={css(styles.userHeader)}>
            ...
            <div className={css(styles.headerLinks)}>
                <nav className={css(styles.navLinks)}>
                    ...
                    <a 
                        href="#" 
                        onClick={handleCompleteLectureClick}
                        className={css(styles.navLink, (additionalBoldLink === '/completeLecture' || additionalBoldLink === '/myCompleteLecture') && styles.bold)}
                    >
                        기이수 과목 관리
                    </a>
