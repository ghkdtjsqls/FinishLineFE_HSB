...
function UserGuide() {
    ...
    const [hasCompletedLectures, setHasCompletedLectures] = useState(false); // localStorage에서 기이수 과목 등록 여부를 확인하는 상태 추가

    // 컴포넌트 마운트 시 로그인 상태와 기이수 과목 등록 여부 확인
    useEffect(() => {
        const loginStatus = localStorage.getItem('isLoggedIn') === 'true';
        const lecturesStatus = localStorage.getItem('hasCompletedLectures') === 'true';
        setIsLoggedIn(loginStatus);
        setHasCompletedLectures(lecturesStatus);
    }, []);
    ...
    const handleSubjectButtonClick = () => {
        if (!isLoggedIn) {
            ...
            // localStorage에서 completedLectures 상태 확인
            const hasCompletedLectures = localStorage.getItem('completedLectures') === 'true';
            if (hasCompletedLectures) {
                navigate('/mycompleteLecture');  // 이미 등록했다면 조회 페이지로 이동
            } else {
                navigate('/completeLecture');    // 미등록 상태라면 등록 페이지로 이동
            }
        }
    };
    ...
    const handleRequirementsButtonClick = () => {
        if (isLoggedIn) {
            navigate('/graduateCheck');  // 로그인 상태일 때 졸업요건 검사 페이지로 이동
        } else {
